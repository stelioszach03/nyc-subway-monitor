...
# Ορισμός της βασικής διεύθυνσης
...
test_endpoint() {
    ...
    response=$(curl -s -I "http://$BASE_URL/$endpoint" | head -1)
    ...
    if [[ "$status" == "200" ]]; then
        ...
        # Get cache status
        cache_status=$(curl -s -I "http://$BASE_URL/$endpoint" | grep -i "X-Cache-Status" | awk '{print $2}')
        echo "  Cache status: $cache_status"
        
        # Verify content type
        content_type=$(curl -s -I "http://$BASE_URL/$endpoint" | grep -i "Content-Type" | awk '{print $2}')
        echo "  Content-Type: $content_type"
        
        # Get content length
        content_length=$(curl -s -I "http://$BASE_URL/$endpoint" | grep -i "Content-Length" | awk '{print $2}')
        echo "  Content-Length: $content_length bytes"
        
        # Get first few bytes to verify it's binary
        first_bytes=$(curl -s "http://$BASE_URL/$endpoint" | head -c 20 | xxd -p | tr -d '\n')
...
for i in {1..10}; do
    echo -n "Request $i: "
    time curl -s -o /dev/null "http://$BASE_URL/gtfs-ace"
    echo ""
done
