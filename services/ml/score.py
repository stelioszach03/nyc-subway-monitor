...
# Define SimpleAnomalyModel at module level so it can be pickled
    ...
    try:
        ...
        if os.path.exists(onnx_path):
            ...
            if model_last_modified is None or current_mtime > model_last_modified:
                print(f"Detected new/updated model at {onnx_path}, reloading...")
                model_last_modified = current_mtime
                reload_model()
        elif os.path.exists(pkl_path):
            current_mtime = os.path.getmtime(pkl_path)
            if model_last_modified is None or current_mtime > model_last_modified:
                print(f"Detected new/updated model at {pkl_path}, reloading...")
                model_last_modified = current_mtime
                reload_model()
...
def reload_model():
    ...
    try:
        ...
        if os.path.exists(onnx_path):
            ...
            print("ONNX model reloaded successfully")
        elif os.path.exists(pkl_path):
            print("Reloading pickle model...")
            with open(pkl_path, 'rb') as f:
                onnx_model = pickle.load(f)
            model_last_modified = os.path.getmtime(pkl_path)
            print("Pickle model reloaded successfully")
        else:
            print("No model found to reload")
            return
        ...
        print("Model, scaler, and feature columns reloaded successfully")
    except Exception as e:
...
async def startup_event():
    ...
    try:
        ...
        if os.path.exists(onnx_path):
            ...
            with open(pkl_path, 'rb') as f:
                onnx_model = pickle.load(f)
            model_last_modified = os.path.getmtime(pkl_path)
            print(f"Pickle model loaded successfully from {pkl_path}")
        elif AUTO_CREATE_MODEL:
            print("No model found. Creating fallback model automatically...")
            onnx_model = create_fallback_model()
            model_last_modified = time.time()
            print("Fallback model created and loaded")
        else:
            print(f"No model found at {onnx_path} or {pkl_path}, using fallback scoring")
            onnx_model = None
        ...
        print(f"Starting model update checking every {MODEL_RELOAD_INTERVAL} seconds")
    ...
    print(f"Model type: {'ONNX' if isinstance(onnx_model, ort.InferenceSession) else 'Custom Model' if onnx_model else 'Fallback'}")
    ...
    try:
        ...
        for key in train_keys:
            try:
                ...
                print(f"Redis error on key {key}: {e}")
                continue
...
def score_anomaly(features: Dict[str, Any]) -> float:
    ...
    try:
        ...
        print(f"Model type: {type(onnx_model)}")
