        ...
        for key in train_keys[:limit]:
            try:
                ...
                try:
                    ...
                    print(f"Error parsing train data for {key}: {e}")
                    continue
...
async def get_alerts(
    ...
    try:
        response = await http_client.get(f"{ML_SERVICE_URL}/anomalies", params=params, timeout=5.0)
...
async def get_subway_metrics(
    ...
    anomaly_scores = {}
    try:
        response = await http_client.get(f"{ML_SERVICE_URL}/scores", timeout=3.0)
        response.raise_for_status()
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        try:
            ...
            print(f"Error sending initial train data: {e}")
...
async def health_check():
    ...
    try:
        ...
        health_data["redis"] = "connected" if redis_status else "error"
    except Exception as e:
        health_data["redis"] = f"error: {str(e)}"
    
    # Check ML service
    try:
        response = await http_client.get(f"{ML_SERVICE_URL}/health", timeout=2.0)
        if response.status_code == 200:
            health_data["ml_service"] = "connected"
        else:
            health_data["ml_service"] = f"error: {response.status_code}"
    except Exception as e:
        health_data["ml_service"] = f"error: {str(e)}"
